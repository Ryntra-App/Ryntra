...
private fun List<com.rinthy.shared.network.AnalyticsResponse>.merge(): com.rinthy.shared.network.AnalyticsResponse {
    if (isEmpty()) return com.rinthy.shared.network.AnalyticsResponse(status = 200)
    return com.rinthy.shared.network.AnalyticsResponse(
...
private fun List<com.rinthy.shared.network.AnalyticsResponse>.firstFailedStatusOrSuccess(): Int =
    firstOrNull { it.status !in 200..299 }?.status ?: firstOrNull()?.status ?: 200

private fun mergeAnalyticsPoints(
    groups: List<List<com.rinthy.shared.model.AnalyticsPoint>>,
): List<com.rinthy.shared.model.AnalyticsPoint> {
    ...
    return List(maxSize) { index ->
        ...
        val mergedProjects = mutableMapOf<String, com.rinthy.shared.model.AnalyticsMetrics>()
        pointsAtIndex.forEach { point ->
            point.projects.forEach { (projectId, metrics) ->
                mergedProjects[projectId] =
                    mergedProjects.getOrElse(projectId) { com.rinthy.shared.model.AnalyticsMetrics() } + metrics
            }
        }
        com.rinthy.shared.model.AnalyticsPoint(
            startTime = pointsAtIndex.firstOrNull()?.startTime.orEmpty(),
            metrics = pointsAtIndex.fold(com.rinthy.shared.model.AnalyticsMetrics()) { total, point ->
                total + point.metrics
            },
