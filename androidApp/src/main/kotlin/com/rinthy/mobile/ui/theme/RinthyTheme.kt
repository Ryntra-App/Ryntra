...
val RinthyGreen = Color(0xFF30D158)
val RinthyCyan = Color(0xFF0A84FF)
private val RinthyDarkColors = darkColorScheme(
    primary = RinthyGreen,
    onPrimary = Color.Black,
    primaryContainer = Color(0xFF2C2C2E),
    onPrimaryContainer = RinthyGreen,
...
private val LocalRinthyMotion = staticCompositionLocalOf { RinthyMotion(isReduced = false) }
...
object RinthyDesign {
    ...
    val motion: RinthyMotion
        @Composable
        @ReadOnlyComposable
        get() = LocalRinthyMotion.current

    val isPlatformNative: Boolean
        @Composable
        @ReadOnlyComposable
        get() = LocalRinthyColors.current.isPlatformNative
...
@Composable
fun RinthyMotionProvider(
    reduceMotion: Boolean,
    content: @Composable () -> Unit,
) {
    CompositionLocalProvider(LocalRinthyMotion provides RinthyMotion(isReduced = reduceMotion), content = content)
}
...
private val RinthyTypography = Typography(
...
fun RinthyTheme(
    ...
    val colorScheme = when {
        !isPlatformNative -> RinthyDarkColors
    ...
    CompositionLocalProvider(LocalRinthyColors provides semanticColors) {
        MaterialTheme(
            ...
            typography = if (isPlatformNative) Typography() else RinthyTypography,
            shapes = if (isPlatformNative) Shapes() else RinthyShapes,
        ) {
