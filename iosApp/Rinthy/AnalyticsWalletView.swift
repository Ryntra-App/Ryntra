...
struct AnalyticsWalletView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 11) {
                Image(systemName: "creditcard.fill")
                    ...
                    .background(Color.rinthyGreen.opacity(0.12), in: RoundedRectangle(cornerRadius: 9))
                VStack(alignment: .leading, spacing: 2) {
                    Text("Modrinth wallet").fontWeight(.semibold)
                    Text(subtitle).font(.caption2).foregroundStyle(.secondary)
                }
                Spacer()
                if isLoading { ProgressView().controlSize(.small).tint(.rinthyGreen) }
            }
            ...
            Link(destination: URL(string: "https://modrinth.com/dashboard/revenue")!) {
                ...
                .padding(.vertical, 12)
                .background(Color.rinthySurfaceRaised, in: RoundedRectangle(cornerRadius: 8))
            }
            .padding(.top, 15)
        }
        .padding(15)
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 10).stroke(Color.rinthySeparator, lineWidth: 0.5)
        }
    }
...
struct AnalyticsPayoutHistoryView: View {
    ...
    private func statusColor(_ status: String) -> Color {
        ...
        case "paid", "sent", "completed", "success": return .rinthyGreen
