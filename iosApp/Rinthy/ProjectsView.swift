...
struct ProjectsView: View {
    ...
    private var projectSummary: some View {
        HStack(spacing: 0) {
            ...
            summaryMetric(
                rinthyExactCount(projects.reduce(0) { $0 + $1.downloads }),
                label: "Downloads"
            )
            summaryMetric(
                rinthyExactCount(projects.reduce(0) { $0 + $1.followers }),
                label: "Followers"
            )
        }
        .padding(.vertical, 13)
    ...
    private func toggleFavorite(_ projectId: String) {
        ...
        if systemReduceMotion || appReduceMotion {
            ...
            withAnimation(RinthyMotion.control, update)
        }
    }
}
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 13) {
                    metric(rinthyExactCount(project.downloads), symbol: "arrow.down", tint: Color.rinthyGreen)
                    metric(rinthyExactCount(project.followers), symbol: "heart", tint: .secondary)
...
func rinthyProjectDate(_ value: String?) -> String? {
